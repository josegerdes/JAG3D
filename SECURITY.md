...
2. **Capability token de curta duracao**: assinado com par de chaves **ES256** dedicado (`LICENSE_TOKEN_PRIVATE_KEY`/`LICENSE_TOKEN_PUBLIC_KEY`, diferente do `JWT_SECRET` da sessao), TTL ~15min, verificavel no navegador via `jose` contra a chave publica buscada em runtime (`GET /api/license/public-key`, cacheada em memoria — deliberadamente NAO inlinada no bundle via `NEXT_PUBLIC_*`, pra nao depender de build ARG do Docker). A chave privada nunca sai do servidor.
   - **Degradacao sem HTTPS**: `crypto.subtle` (usado pela verificacao de assinatura) so existe em contexto seguro do navegador (HTTPS ou `localhost`) — um deploy ainda sem SSL configurado nao tem acesso a ele. Como esta camada e so conveniencia de UX (nunca a fronteira real, ver acima), o cliente cai pra so decodificar e checar a expiracao do token sem validar a assinatura nesse caso, em vez de bloquear o usuario inteiro por falta de SSL. **Configurar SSL no deploy o quanto antes** — sem isso essa camada roda degradada.
